...
import ConsumedHistoryPage from './components/ConsumedHistoryPage';
import { 
  DeleteConfirmModal, 
...
import { isErrorStatus } from './lib/utils';
...
export default function MSDManagementSystem() {
  ...
  const errorComponents = msd.components.filter(c => 
    ...
    isErrorStatus(c.status, c.overtime)
  );
  ...
  const sortedAndFilteredComponents = useMemo(() => {
    ...
    return result.sort((a, b) => {
      ...
      const isAError = isErrorStatus(a.status, a.overtime);
      const isBError = isErrorStatus(b.status, b.overtime);
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans p-6 text-slate-800">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <Header globalErrorCount={globalErrorComponents.length} currentView={currentView} setCurrentView={setCurrentView} />

        {!msd.isConnected && (
          <div className="bg-red-600 text-white p-3 rounded-xl shadow-lg border-2 border-red-700 flex items-center gap-3 animate-pulse">
            <AlertTriangle className="w-5 h-5 flex-shrink-0" />
            <div className="font-bold text-sm">
              SUNUCU BAĞLANTISI KESİLDİ — Veriler güncel olmayabilir. Yeniden bağlanılıyor...
            </div>
          </div>
        )}
        ...
        {currentView === 'main' ? (
          ...
          <ConsumedHistoryPage onBack={() => setCurrentView('main')} />
        )}
      </div>
      ...
      <AddComponentModal 
        ...
        setSolderType={msd.setSolderType}
      ...
      <ErrorModal 
        message={msd.deleteErrorMessage} 
        onClose={() => msd.setDeleteErrorMessage(null)}
      />
    </div>
  );
}
